...
## Week 7: TitanCorp Perimeter Assessment (Subnet Recon)

### 🔍 Scenario
Following the CloudNano acquisition, I was authorized to perform a reconnaissance and vulnerability assessment of the `172.88.0.0/24` subnet. This required identifying live assets, mapping services, and triaging vulnerabilities across multiple web and database targets.

### 🛠️ Methodology & Tools
* **Nmap Subnet Sweeping:** Performed `-sn` ping sweeps to identify live hosts while minimizing network noise.
* **Service Interrogation:** Used `-sV` (Version Detection) to fingerprint software across the DMZ, identifying Nginx and Apache web servers.
* **Web Auditing (Nikto):** Conducted targeted vulnerability scans on identified web servers to uncover misconfigurations and outdated modules.
* **Risk Triage:** Evaluated findings using the **Risk = Likelihood x Impact** model to provide TitanCorp with a prioritized remediation roadmap.

### 🚀 Key Findings
1. **Asset Discovery:** Successfully identified three live assets: two web servers (`.10`, `.20`) and one hardened cache database (`.15`).
2. **Critical Vulnerability:** Identified an outdated Nginx (v1.14.2) server. Due to its age and internet-facing status, this was flagged as the highest priority for remediation.
3. **Configuration Weaknesses:** Discovered active `HTTP TRACE` methods on the Apache server, potentially enabling Cross-Site Tracing (XST) attacks.

### 📁 Artifacts
* [Perimeter Assessment Report](./week-7/Perimeter_Assessment.md) - Full analysis and risk justification.
